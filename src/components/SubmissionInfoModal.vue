...
export default {
  ...
  methods: {
    async loadSubmission() {
      const submissions = await assessmentsSubmissionList(this.fundHash);
      const proposalSubmissions = submissions.filter(submission =>
        submission.json.proposalsId.includes(this.proposalId),
      );
      const txsIds = proposalSubmissions.map(({ tx_id }) => tx_id);
