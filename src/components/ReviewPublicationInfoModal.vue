...
export default {
  ...
  computed: {
    ipfsPath() {
      if (this.publication) {
        return `https://ipfs.io/ipfs/${this.publication.json.reviewsCID}`;
      } else {
        return "";
      }
    },
  ...
  methods: {
    async loadPublication() {
      const publications = await reviewsPublicationList(this.fundHash);
      const assessmentPublications = publications.filter(publication =>
        publication.json.assessmentsId?.includes(this.assessmentId),
      );
      const txsIds = assessmentPublications.map(({ tx_id }) => tx_id);
